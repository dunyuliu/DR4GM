...
class Waveqlab3dConverter:
    ...
    def convert_dataset(self, file_set_name: str, file_paths: Dict[str, Path]) -> Dict:
        ...
        # Trim 2 km off each domain edge: PML/absorbing boundary stations have
        # artificially-suppressed motion and pollute distance-binned σ_ln.
        BOUNDARY_BUFFER_M = 2000.0
        x = all_coords[:, 0]; y = all_coords[:, 1]
        x_min, x_max = x.min(), x.max()
        y_min, y_max = y.min(), y.max()
        keep = (
            (x >= x_min + BOUNDARY_BUFFER_M) & (x <= x_max - BOUNDARY_BUFFER_M) &
            (y >= y_min + BOUNDARY_BUFFER_M) & (y <= y_max - BOUNDARY_BUFFER_M)
        )
        n_dropped = (~keep).sum()
        self.logger.info(
            f"Dropping {n_dropped} stations within {BOUNDARY_BUFFER_M:.0f}m of domain boundary "
            f"(x∈[{x_min/1000:.1f},{x_max/1000:.1f}]km, y∈[{y_min/1000:.1f},{y_max/1000:.1f}]km)"
        )
        all_coords = all_coords[keep]
        all_vel_x = all_vel_x[keep]
        all_vel_y = all_vel_y[keep]
        all_vel_z = all_vel_z[keep]
    ...
    def create_fault_geometry_npz(self) -> str:
        ...
        # Apply rotation if specified (WAVEQLAB3D typically doesn't need rotation)
        ...
        # Add moment magnitude if present
        if 'moment_magnitude' in fault_json:
            fault_data['moment_magnitude'] = np.float32(fault_json['moment_magnitude'])
